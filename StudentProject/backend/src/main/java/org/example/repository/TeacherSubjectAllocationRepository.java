...
public interface TeacherSubjectAllocationRepository extends JpaSpecificationRepository<TeacherSubjectAllocation, Long>{
    ...
    @Query("SELECT SUM(tsa.numberOfLectureGroups) FROM TeacherSubjectAllocation tsa WHERE tsa.subject.abbreviation = :subject AND tsa.semester.code = :semester")
    Float getCoveredLectureGroupsInSemester(@Param("subject") String subject, @Param("semester") String semester);

    @Query("SELECT SUM(tsa.numberOfExerciseGroups) FROM TeacherSubjectAllocation tsa WHERE tsa.subject.abbreviation = :subject AND tsa.semester.code = :semester")
    Float getCoveredExerciseGroupsInSemester(@Param("subject") String subject, @Param("semester") String semester);

    @Query("SELECT SUM(tsa.numberOfLabGroups) FROM TeacherSubjectAllocation tsa WHERE tsa.subject.abbreviation = :subject AND tsa.semester.code = :semester")
